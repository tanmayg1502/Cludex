...
extension CodexService {
    ...
    func buildTurnStartRequestParams(
        ...
        // Include agentId so the bridge can route turn/start to the correct agent transport
        // without relying solely on the server-side session map lookup.
        if let agentId = thread(for: threadId)?.agentId,
