...
// Purpose: Minimal "Start new chat" sheet that lets the user pick a project, worktree, Quick Chat, or Claude.
// Layer: View
...
struct SidebarNewChatProjectPickerSheet: View {
    ...
    var onSelectWithClaude: ((String?) -> Void)? = nil
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                if let onSelectWithClaude {
                    Section("Claude") {
                        Button {
                            dismiss()
                            onSelectWithClaude(nil)
                        } label: {
                            projectRow(
                                icon: AnyView(
                                ...
                                title: "Claude",
                                subtitle: "Start a chat powered by Claude Sonnet."
                            )
                        }
                        .buttonStyle(.plain)
                        ...
                        ForEach(choices) { choice in
                            Button {
                                dismiss()
                                onSelectWithClaude(choice.projectPath)
                            } label: {
                                projectRow(
                                    icon: AnyView(
                                        RemodexIcon.image(systemName: "folder")
                                            .font(AppFont.body(weight: .medium))
                                            .foregroundStyle(.secondary)
                                    ),
                                    title: "\(choice.label)",
                                    subtitle: "Claude in \(choice.label)"
                                )
                            }
                            .buttonStyle(.plain)
                        }
