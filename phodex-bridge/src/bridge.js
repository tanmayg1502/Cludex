...
//   When REMODEX_CLAUDE_CODE_ENABLED=1, also routes Claude Code threads via the agent router.
// Layer: CLI service
// Exports: startBridge
// Depends on: ws, crypto, os, ./bridge-status, ./codex-desktop-refresher, ./agent-router, ./rollout-watch, ./voice-handler
...
function startBridge({
  ...
  const codex = createAgentRouter({ config });
  ...
  function handleApplicationMessage(rawMessage) {
    ...
    // Claude-owned messages bypass all Codex desktop integrations. Without this guard,
    // desktopRefresher.handleInbound would call scheduleNewThreadFallback() for any
    // thread/start without a threadId, opening Codex.app which creates its own UUIDv7
    // thread and races iOS with a thread/started notification before Claude can respond.
    if (isClaudeOwnedMessage(rawMessage)) {
  ...
  function handleBridgeManagedAccountRequest(rawMessage, sendResponse) {
    ...
    if (method !== "account/status/read"
      ...
      && method !== "claude/login/start"
      && method !== "claude/login/status") {
      return false;
    }
  ...
  async function readSanitizedAuthStatus() {
    ...
    const claudeAuthResult = await readClaudeAuthStatus().catch(() => ({ authenticated: false }));
  ...
  let _claudeAuthCacheAt = 0;
  const CLAUDE_AUTH_CACHE_TTL_MS = 30_000;

  async function readClaudeAuthStatus() {
    const now = Date.now();
    if (_claudeAuthCache && (now - _claudeAuthCacheAt) < CLAUDE_AUTH_CACHE_TTL_MS) {
    ...
    try {
      ...
      _claudeAuthCache = {
        authenticated: parsed.authenticated === true || parsed.loggedIn === true,
        email: typeof parsed.email === "string" ? parsed.email : null,
        loginRequired: !(parsed.authenticated === true || parsed.loggedIn === true),
      };
    } catch {
      // claude auth status failed or not installed — treat as unauthenticated.
      _claudeAuthCache = { authenticated: false, loginRequired: true, email: null };
    }

    _claudeAuthCacheAt = Date.now();
    return _claudeAuthCache;
  }

  async function startClaudeLogin() {
    if (process.platform !== "darwin") {
      const error = new Error("Claude login is only supported on macOS.");
      error.errorCode = "unsupported_platform";
      throw error;
    }

    // Invalidate auth cache so the next status check re-reads.
    _claudeAuthCache = null;
    _claudeAuthCacheAt = 0;

    // Spawn claude auth login non-interactively (opens browser).
    ...
    return { ok: true, message: "Claude login opened in browser." };
  }
  ...
  // Returns true for messages that must bypass Codex desktop integrations entirely.
  // Claude-owned messages are identified by an explicit agentId param or a saved session entry.
  function isClaudeOwnedMessage(rawMessage) {
    const parsed = safeParseJSON(rawMessage);
    if (!parsed) return false;
    // Check flat params (inbound thread/start, outbound turn/started, turn/completed, etc.)
    // and nested thread object (outbound thread/started).
    const agentId = parsed?.params?.agentId
      || parsed?.params?.agent_id
      || parsed?.params?.thread?.agentId;
    if (typeof agentId === "string" && agentId.trim().toLowerCase() === "claude-code") {
      return true;
    }
    // For messages without an explicit agentId, fall back to the session map.
    ...
    if (threadId) {
      ...
      if (entry?.agentId === "claude-code") return true;
    }
    return false;
  }
