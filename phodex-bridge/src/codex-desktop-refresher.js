...
class CodexDesktopRefresher {
  ...
  handleInbound(rawMessage) {
    ...
    // Claude-owned messages must not trigger Codex.app desktop refresh or the
    // new-thread fallback — those are Codex-only integrations.
    const agentId = parsed?.params?.agentId || parsed?.params?.agent_id;
  ...
  handleOutbound(rawMessage) {
    ...
    // Claude-originated outbound messages must not trigger Codex.app refresh.
    const outboundAgentId = parsed?.params?.agentId
      || parsed?.params?.agent_id
      || parsed?.params?.thread?.agentId;
    if (typeof outboundAgentId === "string" && outboundAgentId.trim().toLowerCase() === "claude-code") {
      return;
    }
...
function readBridgeConfig({
  ...
  return {
    ...
    permissionTimeoutSecs: parseIntegerEnv(
      readFirstDefinedEnv(["REMODEX_PERMISSION_TIMEOUT_SECS"], "30", env),
      30
    ),
    warmIdleTimeoutSecs: parseIntegerEnv(
      readFirstDefinedEnv(["REMODEX_WARM_IDLE_TIMEOUT_SECS"], "300", env),
      300
    ),
    claudeCodeDefaultPermissionMode: readFirstDefinedEnv(
      ["REMODEX_DEFAULT_PERMISSION_MODE"],
      "acceptEdits",
      env
    ),
  };
